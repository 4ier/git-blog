...
这篇记录一条**用 Pixel 旧机做二级备份**的完整方案：为家里 NAS 上部署的 Immich 建立一条 **Immich 原图 → Pixel 手机 → Google Photos** 的备份通道。重点放在**整套方案的实现路径**（已脱敏），包括目录可识别、自动更新、硬链接节省空间等关键细节。
...
4. Syncthing 分享导出视图（Send Only）到 **Pixel 手机**（Receive Only）。
